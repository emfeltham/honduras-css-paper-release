# rates_2_new.jl
# julia --threads=24 --project="." "honduras-css-paper/code/rates_2_new.jl" > "output/rates_2_new.jl.txt"
...
fx_k = @eval @formula(
    tpr ~ fpr_k + fpr_k^2 + relation + degree + age + age^2 + man
)

fx_l = @eval @formula(
    tpr_l ~ fpr + fpr^2 + relation + degree + age + age^2 + man
)
...
save_object("interaction models/tpr_fpr_τ_new.jld2", τ)
