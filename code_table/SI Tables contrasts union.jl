...
md = load_object("objects/base1_tie2_margins_union.jld2")
...
for e in [
    :age, :degree, :dists_p, :age_mean_a, :age_diff_a,
...
table_export(
    tblpath * "SI Table contrasts union respondent",
    tbx;
    extra,
    placement = :auto,
    short_caption = "Contrasts for respondent and village characteristics, union",
...
table_export(
    tblpath * "SI Table contrasts union tie",
    tbx;
    extra,
    placement = :auto,
    short_caption = "Contrasts for tie characteristics, union",
    caption = "Contrasts for tie characteristics, union. Each accuracy measure represents the difference between the predicted value for each level of the contrast. 95% confidence intervals are presented in parentheses below the mean estimates. The corresponding p-values are presented below the intervals, and are rounded to 5 digits.",
    kind,
    supplement,
    numbering = "1",
    gap = Symbol("0.65em"),
